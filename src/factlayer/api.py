...
@app.get("/health")
def health() -> dict:
    ready = llm.available_providers()
    return {"status": "ok", "providers_ready": ready, "uploads_enabled": bool(ready)}
...
async def upload(
    ...
    try:
        ...
        # A public deployment can ship a prepared corpus with no key attached.
        # Saying so beats accepting a PDF and storing nothing from it.
